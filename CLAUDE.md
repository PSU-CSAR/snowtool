...
pytest tests/snowdb/test_db.py::test_open_requires_a_root_config   # single test
...
uvicorn snowtool.api.app:get_app --factory --reload   # dev API server (no module-level app)
...
**Datasets are data, not subclasses.** A `DatasetSpec` (`snowdb/spec.py`) carries a grid, its `DatasetVariable`s, and an `Ingester`. Built-in specs live in `snowdb/datasets/` (snodas, swann, instarr) and are collected into `DEFAULT_DATASET_SPECS`, which backs `DATASET_TEMPLATES` and the tests — not a `SnowDb` directly. Adding a dataset is a new spec + ingester, registered in that package — no new dataset class. Specs reach a `SnowDb` as inline or path config links (`snowdb/config.py`); ingest turns a source artifact into per-date COGs under `data/<name>/cogs/<YYYYMMDD>/`.

**Zone layers are pluggable along three axes** — read these together, they interlock:
- `ZoneLayerProvider` (terrain, landcover) generates derived layers (`snowdb/zones/terrain.py`, `landcover.py`); the built-ins are `DEFAULT_ZONE_LAYER_PROVIDERS` in `snowdb/zones/zone_layer_providers.py`, passed to the `SnowDb` constructor (CLI: `CliContext.zone_layer_providers`).
- `ZoneLayerSource` is *where the input comes from* (`DemSource`→`LocalFile`/`ThreeDEP`; `LandCoverSource`→`LocalFile`/`AnnualNLCD`). Sources are **injected** — declared in the root config's `sources` map or overridden per command with `--source` — so tests use local files instead of S3.
- `ZoneScheme` (`snowdb/zones/zoning.py`) turns a layer's pixel values into zones: `BandedZoning` (elevation bands), `ThresholdZoning` (forest above/below %), `CategoricalZoning` (aspect classes).
...
**Provenance is a versioned hash.** Every derived artifact is tagged `v{format_version}:{sha256}` (`snowdb/provenance.py`); a content change *or* a format-version bump reads as stale and forces a rebuild via the same equality check. Tags: `SNOWTOOL_TILE_BBOX`, `SNOWTOOL_AOI_HASH`, `SNOWTOOL_DEM_HASH`, `SNOWTOOL_NLCD_HASH` (`snowdb/constants.py`). This is a greenfield DB — versions start at 1 and there is no migration machinery.

## Test writing guidelines

The suite favors **real-code integration on a tiny synthetic grid** over isolated units. Match these conventions:

- **Use the synthetic-grid fixtures** in `tests/conftest.py` (`spec`, `grid`, `dataset`, `swe_cog`, `pourpoint_geojson`, plus `write_terrain`/`write_landcover` helpers). Everything runs on a 512×512 / 2×2-tile geographic grid with hand-computable uniform values, exercising real rasterio/griffine with no system GDAL. CLI tests add `runner`/`cli_obj`/`initialized_root` in `tests/cli/conftest.py` and drive real commands via `runner.invoke(cli, args, obj=cli_obj)`.
