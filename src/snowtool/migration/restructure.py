...
``dem.tif``, ``cogs/``). The snowdb root is ``pourpoints/`` + ``data/<name>/`` per
dataset. This relocates a flat directory to ``data/<dataset>/`` and creates the
global ``pourpoints/`` directory. Driven by ``snowtool migration restructure``.
"""
...
def restructure_to_snowdb(src: Path, dst_root: Path, dataset: str) -> Path:
    ...
    Also creates the global ``dst_root/pourpoints/`` directory. Returns the new
    ...
    (dst_root / 'pourpoints').mkdir(parents=True, exist_ok=True)
