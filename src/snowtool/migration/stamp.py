...
A snowdb built before the root config existed is a bare ``pourpoints/`` + ``data/``
...
def stamp_root(root: Path) -> tuple[Path, bool]:
    ...
    ``root`` is not a directory or lacks the base ``pourpoints/``/``data/`` layout (it
    would not be a snowdb to stamp).
    """
    root = Path(root)
    if not root.is_dir():
        raise ValueError(f'Not a directory: {root}')
    missing = [name for name in ('pourpoints', 'data') if not (root / name).is_dir()]
    if missing:
        raise ValueError(
