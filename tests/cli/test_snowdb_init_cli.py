...
def test_init_creates_layout_and_terrain(
    ...
    assert 'generating land cover (default source)' in result.output
...
def test_init_dataset_dem_override(runner, tmp_path, spec, source_dem, source_nlcd):
    root = tmp_path / 'db'
    result = runner.invoke(
        cli,
...
def test_init_dataset_nlcd_override(runner, tmp_path, spec, source_dem, source_nlcd):
    root = tmp_path / 'db'
    result = runner.invoke(
        cli,
        ['snowdb', 'init', str(root), '--dataset-nlcd', 'test', str(source_nlcd)],
        obj=_ctx(root, spec, source_dem, source_nlcd),
    )

    assert result.exit_code == 0
    assert f'generating land cover for test from {source_nlcd}' in result.output
    assert 'generating land cover (default source)' not in result.output
    assert (root / 'data' / 'test' / 'landcover' / 'forest_cover_pct.tif').is_file()
...
def test_init_is_idempotent(runner, tmp_path, spec, source_dem, source_nlcd):
    ...
    assert 'generating land cover' not in second.output
